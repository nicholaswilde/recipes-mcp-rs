...
use crate::search::{RecipeProvider, SearchResult};
...
impl RecipeCache for FileRecipeCache {
    async fn get_search_results(
        ...
        let entry = fs::read_to_string(&path).ok().and_then(|content| {
            serde_json::from_str::<CacheEntry<Vec<SearchResult>>>(&content).ok()
        });
    ...
    async fn get_recipe(&self, url: &str) -> Option<Recipe> {
        ...
        let entry = fs::read_to_string(&path)
            .ok()
            .and_then(|content| serde_json::from_str::<CacheEntry<Recipe>>(&content).ok());
        ...
        if let Ok(entries) = fs::read_dir(&self.cache_dir) {
            for entry in entries.flatten() {
                ...
                if path.is_file() && path.extension().and_then(|s| s.to_str()) == Some("json") {
                    let entry = fs::read_to_string(&path).ok().and_then(|content| {
                        serde_json::from_str::<CacheEntry<serde_json::Value>>(&content).ok()
                    });

                    if entry.filter(|e| e.is_expired()).is_some() {
                        let _ = fs::remove_file(path);
                    }
                }
            }
...
mod tests {
    ...
    async fn test_file_cache_recipe() {
        ...
        // Set recipe
        cache
            .set_recipe(url, recipe.clone(), Duration::from_secs(60))
            .await;
    ...
    async fn test_cache_expiration() {
        ...
        // Set with 0 TTL
        cache
            .set_recipe(url, recipe.clone(), Duration::from_secs(0))
            .await;
